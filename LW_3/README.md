...
# [Материалы лабораторной работы №3](https://github.com/alexey-malov/oop/tree/master/tasks/03)
...
| Возвращаемый тип | Функция | Описание |
|----------|----------|----------|
...
`SetGear` передача.
- Включает заданную передачу, заданную числом от -1 до 5. В случае ошибки сообщает почему невозможно переключить передачу.

`SetSpeed` скорость.
- Устанавливает указанную скорость движения, заданную неотрицательным числом. В случае невозможности изменения скорости сообщает, почему нельзя изменить скорость на указанную.